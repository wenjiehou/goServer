...
var (
	...
	SelfPoints    = []int{5, 9}
	IFCNingBoType = reflect.TypeOf(&RoomNingBo{})

	Jiaofen    = 101 //=(清12混8)
	Qiangdizhu = 102 //=(清10混6
)
...
const (
	...
	JiaofenTime = 10 //当前玩家超过15s没有叫，自动不叫
	JiabeiTime  = 10
	DapaiTime   = 20
	ReayTime    = 10
)
...
// 西周规则
var NingBo_RoomRule = rb.RoomRule{
	GameType:           4000,         //
	Create_NeedDiamond: 100,          // 创建房间需要的钻石
	SeatLen:            3,            // 座位数量  2, 3, 4
...
type RoomNingBo struct {
	...
	JiaoType         int                      // 叫地主类型   101 叫分 102 抢地主
...
// 座位
type Seat_NingBo struct {
	rb.SeatBase
}

// 初始化
func (r *RoomNingBo) Init() {
	r.RoomBase.Init()
...
func (r *RoomNingBo) TimeTicker() {
	...
	for {
		...
		if r.Stage == Stage_jiaofen {
			if r.LeftTime >= JiaofenTime || (r.Seats[r.CurIndex] != nil && r.Seats[r.CurIndex].User.IsRobot == 1) {
				if r.CostType == rb.Jinbi {
					if r.Seats[r.CurIndex] != nil && r.Seats[r.CurIndex].HaveJiao == false {
						r.Jiaofen(r.Seats[r.CurIndex].User, 0) //超时不叫
					}
				}
			}
		} else if r.Stage == Stage_jiabei {
			if r.CostType == rb.Jinbi {
				//没有选择加倍的家伙都不加倍
				for _, s := range r.Seats {
					if s != nil && s.HaveJiabei == false {
						if r.LeftTime >= JiabeiTime || s.User.IsRobot == 1 {
							r.Jiabei(s.User, false)
						}
		...
		} else if r.Stage == Stage_dapai { //机器人的逻辑重点写这里
			if r.LeftTime >= DapaiTime || (r.Seats[r.CurIndex] != nil && r.Seats[r.CurIndex].User.IsRobot == 1) {
				if r.CostType == rb.Jinbi {
					//如果当前没有玩家出过牌 或者当前出牌的玩家是自己
					if r.CurOutputIdx == r.CurIndex || r.CurOutputIdx == -1 {
						if r.Seats[r.CurIndex] != nil {
							prePos := (r.CurIndex + r.Rules.SeatLen - 1) % r.Rules.SeatLen
							nexPos := (r.CurIndex + r.Rules.SeatLen + 1) % r.Rules.SeatLen

							preArr := r.GetIntArray(r.Seats[prePos].Cards.List)
							nexArr := r.GetIntArray(r.Seats[nexPos].Cards.List)

							others := append(preArr, nexArr...)

							preStep := r.Seats[prePos].Cards.OutStep
							nexStep := r.Seats[nexPos].Cards.OutStep
							...
							//1 1 1 2 2 2 3 3 4 4
							paixing := px.GetPaixing(retArr)
							cards := r.GetInt32Arr(retArr)

							if paixing.Paixing != px.PAIXING_NON {
								r.Chupai(r.Seats[r.CurIndex].User, &mjgame.Sddz_Chupai{
									Sid:   r.Seats[r.CurIndex].User.Sid,
									Type:  int32(paixing.Paixing),
									Cards: cards,
								})
							} else {
						...
						if r.Seats[r.CurIndex] != nil {
							...
							selfStep := r.Seats[r.CurIndex].Cards.OutStep
							preStep := r.Seats[prePos].Cards.OutStep
							nexStep := r.Seats[nexPos].Cards.OutStep

							var outIdx = 0

							if r.CurOutputIdx == prePos {
								outIdx = -1
							} else if r.CurOutputIdx == nexPos {
								outIdx = 1
							}

							out := r.GetIntArr(r.CurOutputCards.Cards)
							var retArr []int

							if r.CurIndex == r.DizhuPos { //我是地主
								retArr = px.DizhuGen(r.GetIntArray(r.Seats[r.CurIndex].Cards.List), len(preArr), len(nexArr), others, selfStep, preStep, nexStep, outIdx, out)
							} else if nexPos == r.DizhuPos { //我是地主上家
								retArr = px.DizhuSGen(r.GetIntArray(r.Seats[r.CurIndex].Cards.List), preArr, nexArr, others, selfStep, preStep, nexStep, outIdx, out)
							} else if prePos == r.DizhuPos { //我是地主下家
								retArr = px.DizhuXGen(r.GetIntArray(r.Seats[r.CurIndex].Cards.List), preArr, nexArr, others, selfStep, preStep, nexStep, outIdx, out)
							}
							if len(retArr) > 0 { //有大过的
								if len(px.GetBigthan(retArr, out)) > 0 {
									paixing := px.GetPaixing(retArr)
									...
									if paixing.Paixing != px.PAIXING_NON {
										...
										fmt.Println("大过的牌型有问题cards::", retArr)
									}

								} else {
									fmt.Println("返回大过的牌有问题cards::", retArr)
								}
							} else { //要不起
								r.Sddz_Pass(r.Seats[r.CurIndex].User)
							}
		...
		} else if r.Stage == Stage_End && r.RoundCount > 0 {

			//自动准备（准备）
			if r.CostType == rb.Jinbi {
				for _, v := range r.Seats {
					if r.LeftTime >= ReayTime || v.User.IsRobot == 1 {
...
// 检查是否可以开始
func (r *RoomNingBo) CheckCanStart() (bool, *mjgame.ErrorItem) {
...
// 添加承包关系
func (r *RoomNingBo) AddChengBao(index int, tindex int) {
...
func (r *RoomNingBo) RoundTotal() {
	...
	if r.StlCtrl != nil && (r.StlCtrl).(*NingBo_Statement) != nil {
		lastTool = (r.StlCtrl).(*NingBo_Statement).Get(0)
	}
...
// 获得开始信息  开始叫分的位置
//func (r *RoomNingBo) GetStartInfo() (int, int, int, int) {
...
// 改变用户座位
func (r *RoomNingBo) ChangeUsersPosition() []*mjgame.ACK_User_Info {
	seed := rand.New(rand.NewSource(time.Now().UnixNano()))
...
// 算番
func (r *RoomNingBo) FanCalc(index int, cardid int) st.TotalResult {
	...
	(r.StlCtrl).(*NingBo_Statement).BaseCtl.SiChuan = false
	return (r.StlCtrl).(*NingBo_Statement).FanCalc(index, arg)
}

//检查是否胡牌
func (r *RoomNingBo) CheckHu(index int, cardId int) int {
	arrMj := r.Seats[index].Cards.List
	...
	result := (r.StlCtrl).(*NingBo_Statement).CheckHu(mjs)
	//fmt.Println(result, "CheckHu result")
	if result > 0 {
		isWin := (r.StlCtrl).(*NingBo_Statement).CheckCanWin(index)
		if !isWin {
			result = 0
		}
	}
...
//添加操作
func (r *RoomNingBo) AddTool(toolType int, index int, tindex int, val []int) {
	(r.StlCtrl).(*NingBo_Statement).AddTool(toolType, index, tindex, val)
}

func (r *RoomNingBo) AddListCard(index int, listcard *al.ArrayList) {
...
//吃
func (r *RoomNingBo) CheckCanChow(index int, tIndex int, cards []*rb.Card, card *rb.Card) bool {
...
//判断是否可以碰牌
func (r *RoomNingBo) CheckPeng(pCard *rb.Card) {
	index := r.CurIndex
	for i := 0; i < 3; i++ {
		index++ //从下家开始判断
...
// forward = 从后面摸牌
// pass = 是否过掉本次操作
func (r *RoomNingBo) TurnNextPlayer() {
	r.CurIndex++
	...
	case Stage_jiaofen:
		...
		ack := mjgame.ACKBC_Sddz_JiaoDizhu{Uid: int32(uid), CurFen: int32(r.Difen), LeftTime: int32(JiabeiTime)}
	...
	case Stage_dapai:
		r.LeftTime = 0
		ack := mjgame.ACKBC_CurPlayer{Seat: int32(r.Seats[r.CurIndex].Index), Type: int32(rb.WaitPut), RoundTime: int32(r.WaitPutTimeOut), LeftTime: int32(DapaiTime)}
...
//等待操作,自摸判断操作
func (r *RoomNingBo) WaitPut(timeout int) {

}

//开始等待操作
func (r *RoomNingBo) StartWaitTool(card *rb.Card) {
	r.WaitOptTool.ClearAll()
...
//检查是否可以胡
func (r *RoomNingBo) CheckWin(cid int) {
	index := r.CurIndex
...
//是否流局
func (r *RoomNingBo) CheckDraw() bool {
...
//计算风圈
func (r *RoomNingBo) CalcDirection() {
	var flag bool
...
//统计
func (r *RoomNingBo) GetSummaryList() mjgame.TotalSummary {
...
//是否可以解散
func (r *RoomNingBo) IsDisbanding() int {
	var agreeCount int
...
func (r *RoomNingBo) InsertRoomRecord(scores []int32) {
	...
	r.RoomBase.Try(func() {
		...
		for k, v := range result {
			roomRecord := &model.RoomRecord{
				...
				RoomType:       int(mjgame.MsgID_GTYPE_SanDizhu),
			}
			if v > 0 {
		...
		fmt.Println("ningbo InsertRoomRecord ", e)
	})
}

//进入房间
func (r *RoomNingBo) IntoRoom(user *user.User) {
	r.Mux.Lock()
	defer r.Mux.Unlock()
	...
	} else if r.Stage == Stage_jiaofen {
		leftTime = JiaofenTime - r.LeftTime
	} else if r.Stage == Stage_jiabei {
		leftTime = JiabeiTime - r.LeftTime
	} else if r.Stage == Stage_dapai {
		leftTime = DapaiTime - r.LeftTime
	}
	...
	ack := &mjgame.ACKBC_Into_Room{
		...
		Type:    int32(mjgame.MsgID_GTYPE_SanDizhu),
...
// 发送玩家牌消息
func (r *RoomNingBo) SendSeatCard(userId int) {
...
func (r *RoomNingBo) SendGameInfo(a *user.User, needRecord bool) {
	...
	fmt.Println("r.DizhuPos", r.DizhuPos)
	...
	ack := &mjgame.ACKBC_Sddz_Card_Init{
		...
		CurTime:        int32(leftTime), //
		DizhuIndex:     int32(r.DizhuPos),
...
//得到房间信息
func (r *RoomNingBo) GetRoomInfo() *mjgame.ACK_Room_Info {
...
func (r *RoomNingBo) GetRecordRoomInfo() interface{} {
	ack := &struct {
		RID        int32
		Type       int32
		Rule       []int32
...
//是否可以解散
func (r *RoomNingBo) GetDisApproveCount() int {
	var count int
	for _, v := range r.Votes {
		if v == DisApprove {
			count++
...
func (r *RoomNingBo) GetMultiWinInfo() ([]int, int) {
	//四人麻将最多三个人胡
	var huIndexes []int
	var huCardId = -1

	for i := 0; i < 3; i++ {
		lastTool := (r.StlCtrl).(*NingBo_Statement).Get(i)
...
func (r *RoomNingBo) GetMaxIndex(arr []int32) int {
	var maxIndex int
	var score int32
...
// 当前局可结束
func (r *RoomNingBo) RoundCanFinish() bool {
	var count int
...
// 以局数计算两局之间可否结束
func (r *RoomNingBo) ByRoundCanFinish() bool {
...
func (r *RoomNingBo) GetKickIndex() []int32 {
	var indexs []int32
	for k, v := range r.Seats {
		if v.User != nil {
			if v.User.State == def.Offline {
				if r.CostType == rb.Jinbi {
					if v.OfflineTime.Add(def.JinBiKickTime*time.Second).Unix() == time.Now().Unix() {
...
// 杠牌操作
func (r *RoomNingBo) MoveKongList(uIndex int, tIndex int, cid, kongType int) {
	seat := r.Seats[uIndex]
...
func (r *RoomNingBo) MoveChowList(index int, cards []*rb.Card) {
	seat := r.Seats[index]
...
func (r *RoomNingBo) DealKongHu(cardID int) {
...
func (r *RoomNingBo) ClearRoomUserRoomId() {
	for _, v := range r.Seats {
		if v.User != nil {
			v.User.RoomId = 0
			v.User.GameType = nil
...
//设置玩家手中牌 用于测试环境
func (r *RoomNingBo) SetInitCards(uid string, cids []string) {
	for _, v := range r.Seats {
...
//得到房间用户信息
func (r *RoomNingBo) GetRoomUser() *mjgame.ACK_Room_User {
...
//判断是否可以吃牌
func (r *RoomNingBo) CheckChow(pCard *rb.Card) {
...
//设置不能吃的牌
func (r *RoomNingBo) SetPassChowCards(pCard *rb.Card) {
