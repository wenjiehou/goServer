...
//  不是正花1台
//  (8花是覆盖关系)
func (sc *XiZhou_Statement) F_Hua(seatIndex int) *st.FanResult {
	...
	room := (*sc.BaseCtl.RoomRef).(*RoomXiZhou)
...
//
func (sc *XiZhou_Statement) F_Feng(seatIndex int) *st.FanResult {
	tp := 3002
	tai := 0 //台数
	msg := ""
	SpecialSid := -1 //用于特别的算番记录的uid，例如 拉杠胡，送杠胡，还杠胡，其它用不到的传-1
...
func (sc *XiZhou_Statement) F_ZhongFaBai(seatIndex int) *st.FanResult {
	tp := 3003 //
	tai := 0
	msg := ""
	SpecialSid := -1 //用于特别的算番记录的uid，例如 拉杠胡，送杠胡，还杠胡，其它用不到的传-1
...
// 【天胡】 24台，庄家起手胡牌；
func (sc *XiZhou_Statement) F_TianHu(seatIndex int) *st.FanResult {
	tp := 3004 //
	msg := ""
	tai := 0
	SpecialSid := -1 //用于特别的算番记录的uid，例如 拉杠胡，送杠胡，还杠胡，其它用不到的传-1
...
// 【地胡】 16台，庄家打出的那张牌有人胡掉；
func (sc *XiZhou_Statement) F_DiHu(seatIndex int) *st.FanResult {
	tp := 3005 //
	msg := ""
	tai := 0
	SpecialSid := -1 //用于特别的算番记录的uid，例如 拉杠胡，送杠胡，还杠胡，其它用不到的传-1
...
func (sc *XiZhou_Statement) F_LaGangHu(seatIndex int) *st.FanResult {
	tp := 3006 //
	msg := ""
	tai := 0
	SpecialSid := -1 //用于特别的算番记录的uid，例如 拉杠胡，送杠胡，还杠胡，其它用不到的传-1
...
func (sc *XiZhou_Statement) F_SongGangHu(seatIndex int) *st.FanResult {
	tp := 3007 //
	msg := ""
	tai := 0
	SpecialSid := -1 //用于特别的算番记录的uid，例如 拉杠胡，送杠胡，还杠胡，其它用不到的传-1
...
func (sc *XiZhou_Statement) F_KangShangKaiHua(seatIndex int) *st.FanResult {
	tp := 3008 //
	msg := ""
	tai := 0
	SpecialSid := -1 //用于特别的算番记录的uid，例如 拉杠胡，送杠胡，还杠胡，其它用不到的传-1
...
func (sc *XiZhou_Statement) F_HaiDiLaoYue(seatIndex int) *st.FanResult {
	tp := 3009 //
	msg := ""
	tai := 0
	SpecialSid := -1 //用于特别的算番记录的uid，例如 拉杠胡，送杠胡，还杠胡，其它用不到的传-1
...
func (sc *XiZhou_Statement) F_BianDao(seatIndex int) *st.FanResult {
	// 边倒只有3, 7可以
	tp := 3010
	msg := ""
	tai := 0
...
func (sc *XiZhou_Statement) F_QianDao(seatIndex int) *st.FanResult {
	// 嵌倒 2345678
	tp := 3011 //
	msg := ""
	tai := 0
...
func (sc *XiZhou_Statement) F_DanDiao(seatIndex int) *st.FanResult {
	tp := 3012 //
	msg := ""
	tai := 0
	SpecialSid := -1 //用于特别的算番记录的uid，例如 拉杠胡，送杠胡，还杠胡，其它用不到的传-1
...
func (sc *XiZhou_Statement) F_DuiDao(seatIndex int) *st.FanResult {
	tp := 3013 //
	msg := ""
	tai := 0
	SpecialSid := -1 //用于特别的算番记录的uid，例如 拉杠胡，送杠胡，还杠胡，其它用不到的传-1
...
func (sc *XiZhou_Statement) F_Duan19(seatIndex int) *st.FanResult {
	tp := 3014             //
	mjs := make([]int, 42) // 麻将
	SpecialSid := -1       //用于特别的算番记录的uid，例如 拉杠胡，送杠胡，还杠胡，其它用不到的传-1
	room := (*sc.BaseCtl.RoomRef).(*RoomXiZhou)
...
// 【自摸】1台，
func (sc *XiZhou_Statement) F_ZiMo(seatIndex int) *st.FanResult {
	tp := 3015 //
	tai := 0
	msg := ""
	SpecialSid := -1 //用于特别的算番记录的uid，例如 拉杠胡，送杠胡，还杠胡，其它用不到的传-1
...
func (sc *XiZhou_Statement) F_MenQing(seatIndex int) *st.FanResult {
	tp := 3016 //
	tai := 0
	msg := ""
	SpecialSid := -1 //用于特别的算番记录的uid，例如 拉杠胡，送杠胡，还杠胡，其它用不到的传-1
...
// 【大吊车】8台，手上只剩一张牌，
func (sc *XiZhou_Statement) F_DaDiaoChe(seatIndex int) *st.FanResult {
	tp := 3017 //
	msg := ""
	tai := 0
	SpecialSid := -1 //用于特别的算番记录的uid，例如 拉杠胡，送杠胡，还杠胡，其它用不到的传-1
...
func (sc *XiZhou_Statement) F_HunYiSe(seatIndex int) *st.FanResult {
	tp := 3018 //
	msg := ""
	tai := 0
	SpecialSid := -1 //用于特别的算番记录的uid，例如 拉杠胡，送杠胡，还杠胡，其它用不到的传-1
...
func (sc *XiZhou_Statement) F_DuiDuiHu(seatIndex int) *st.FanResult {
	tp := 3019       //
	SpecialSid := -1 //用于特别的算番记录的uid，例如 拉杠胡，送杠胡，还杠胡，其它用不到的传-1
	room := (*sc.BaseCtl.RoomRef).(*RoomXiZhou)
...
func (sc *XiZhou_Statement) F_QuanShunZi(seatIndex int) *st.FanResult {
	tp := 3020 //
	msg := ""
	tai := 0
	SpecialSid := -1 //用于特别的算番记录的uid，例如 拉杠胡，送杠胡，还杠胡，其它用不到的传-1
...
// 【清一色】12台，胡牌的牌全由一色牌组成；
func (sc *XiZhou_Statement) F_QinYiSe(seatIndex int) *st.FanResult {
	tp := 3021             //
	SpecialSid := -1       //用于特别的算番记录的uid，例如 拉杠胡，送杠胡，还杠胡，其它用不到的传-1
	mjs := make([]int, 42) // 麻将
	room := (*sc.BaseCtl.RoomRef).(*RoomXiZhou)
...
func (sc *XiZhou_Statement) F_FengYiSe(seatIndex int) *st.FanResult {
	tp := 3022 //
	msg := ""
	tai := 0
	SpecialSid := -1 //用于特别的算番记录的uid，例如 拉杠胡，送杠胡，还杠胡，其它用不到的传-1
...
func (sc *XiZhou_Statement) F_BanGao(seatIndex int) *st.FanResult {
	tp := 3023 //
	msg := ""
	tai := 0
	SpecialSid := -1 //用于特别的算番记录的uid，例如 拉杠胡，送杠胡，还杠胡，其它用不到的传-1
		...
		for m := 0; m < 7; m++ {
			if mjs[i+a+m] >= 2 && mjs[i+b+m] >= 2 && mjs[i+c+m] >= 2 {
				...
				if mjs[i+a+m] == 4 && mjs[i+b+m] == 4 && mjs[i+c+m] == 4 {
					...
					mjcache[i+b+m] -= 4
					mjcache[i+c+m] -= 4
					result := (room.StlCtrl).(*XiZhou_Statement).CheckHu(mjcache)
					if result > 0 {
						shun3 = 2 // 4顺子
						m += 2
					}
...
func (sc *XiZhou_Statement) F_DaSiXi(seatIndex int) *st.FanResult {
	tp := 3024 //
	msg := ""
	tai := 0
	SpecialSid := -1 //用于特别的算番记录的uid，例如 拉杠胡，送杠胡，还杠胡，其它用不到的传-1
...
func (sc *XiZhou_Statement) F_HuanGangHu(seatIndex int) *st.FanResult {
	tp := 3025 //
	msg := ""
	tai := 0
	SpecialSid := -1 //用于特别的算番记录的uid，例如 拉杠胡，送杠胡，还杠胡，其它用不到的传-1
...
// 重复关系计算
func (sc *XiZhou_Statement) F_RepeatCalc(fanlist al.ArrayList) al.ArrayList {
...
func (sc *XiZhou_Statement) GetIsDoHu() bool {
	or := sc.Get(0)
