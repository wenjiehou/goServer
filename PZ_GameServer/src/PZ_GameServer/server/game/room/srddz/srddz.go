...
var (
	...
	SelfPoints   = []int{5, 9}
	IFCSrddzType = reflect.TypeOf(&RoomSrddz{})
...
// 西周规则
var Srddz_RoomRule = rb.RoomRule{
	GameType:           4001,        //
...
type RoomSrddz struct {
	...
	IsTouliao        bool                     //是否头撂
	IsMingW          bool                     //是否明王
	BaodaoState      int                      //报到 0不报到 1报到 2双报到
	BaodaoNum        int                      //报到的牌数量
	IsStrictWin      bool                     //是否直接赢
	IsSeleBaodao     bool                     //是否选择过报到
	IsSeleStrictWin  bool                     //是否选择过直接赢
	BaodaoCards      []int32                  //报道的牌
}
...
// 座位
type Seat_Srddz struct {
	rb.SeatBase
}

// 初始化
func (r *RoomSrddz) Init() {
	r.RoomBase.Init()
	...
	r.BaodaoNum = 0
	r.IsStrictWin = false
	r.IsSeleBaodao = false
	r.IsSeleStrictWin = false
	r.BaodaoCards = []int32{}
...
// 检查是否可以开始
func (r *RoomSrddz) CheckCanStart() (bool, *mjgame.ErrorItem) {
...
// 添加承包关系
func (r *RoomSrddz) AddChengBao(index int, tindex int) {
...
func (r *RoomSrddz) RoundTotal() {
	...
	if r.StlCtrl != nil && (r.StlCtrl).(*Srddz_Statement) != nil {
		lastTool = (r.StlCtrl).(*Srddz_Statement).Get(0)
	}
...
// 获得开始信息  开始叫分的位置
//func (r *RoomSrddz) GetStartInfo() (int, int, int, int) {
...
// 改变用户座位
func (r *RoomSrddz) ChangeUsersPosition() []*mjgame.ACK_User_Info {
	seed := rand.New(rand.NewSource(time.Now().UnixNano()))
...
// 算番
func (r *RoomSrddz) FanCalc(index int, cardid int) st.TotalResult {
	...
	(r.StlCtrl).(*Srddz_Statement).BaseCtl.SiChuan = false
	return (r.StlCtrl).(*Srddz_Statement).FanCalc(index, arg)
}

//检查是否胡牌
func (r *RoomSrddz) CheckHu(index int, cardId int) int {
	arrMj := r.Seats[index].Cards.List
	...
	result := (r.StlCtrl).(*Srddz_Statement).CheckHu(mjs)
	//fmt.Println(result, "CheckHu result")
	if result > 0 {
		isWin := (r.StlCtrl).(*Srddz_Statement).CheckCanWin(index)
		if !isWin {
			result = 0
		}
	}
...
//添加操作
func (r *RoomSrddz) AddTool(toolType int, index int, tindex int, val []int) {
	(r.StlCtrl).(*Srddz_Statement).AddTool(toolType, index, tindex, val)
}

func (r *RoomSrddz) AddListCard(index int, listcard *al.ArrayList) {
...
//吃
func (r *RoomSrddz) CheckCanChow(index int, tIndex int, cards []*rb.Card, card *rb.Card) bool {
...
//判断是否可以碰牌
func (r *RoomSrddz) CheckPeng(pCard *rb.Card) {
	index := r.CurIndex
	for i := 0; i < 3; i++ {
		index++ //从下家开始判断
...
// forward = 从后面摸牌
// pass = 是否过掉本次操作
func (r *RoomSrddz) TurnNextPlayer() {
	r.CurIndex++
...
//等待操作,自摸判断操作
func (r *RoomSrddz) WaitPut(timeout int) {

}

//开始等待操作
func (r *RoomSrddz) StartWaitTool(card *rb.Card) {
	r.WaitOptTool.ClearAll()
...
//检查是否可以胡
func (r *RoomSrddz) CheckWin(cid int) {
	index := r.CurIndex
...
//是否流局
func (r *RoomSrddz) CheckDraw() bool {
...
//计算风圈
func (r *RoomSrddz) CalcDirection() {
	var flag bool
...
//统计
func (r *RoomSrddz) GetSummaryList() mjgame.TotalSummary {
...
//是否可以解散
func (r *RoomSrddz) IsDisbanding() int {
	var agreeCount int
...
func (r *RoomSrddz) InsertRoomRecord(scores []int32) {
	...
	r.RoomBase.Try(func() {
		...
		for k, v := range result {
			roomRecord := &model.RoomRecord{
				...
				RoomType:       int(mjgame.MsgID_GTYPE_SirenDizhu),
			}
			if v > 0 {
...
//进入房间
func (r *RoomSrddz) IntoRoom(user *user.User) {
	var isWatch bool
	...
	ack := &mjgame.ACKBC_Into_Room{
		...
		Type:    int32(mjgame.MsgID_GTYPE_SirenDizhu),
...
// 发送玩家牌消息
func (r *RoomSrddz) SendSeatCard(userId int) {
...
func (r *RoomSrddz) SendGameInfo(a *user.User, needRecord bool) {
	...
	if a != nil {
		idx := r.GetSeatIndexById(a.ID)
		if r.DizhuPos != -1 && r.DizhuPos == idx {
			for i := 0; i < len(r.Dipai); i++ {
				fmt.Print("r.Dipai i::", i)
	...
	outZha := make([]int32, r.Rules.SeatLen)
	...
	for i := 0; i < r.Rules.SeatLen; i++ {
		...
		outZha[i] = int32(r.Seats[i].OutZha)
	...
	ack := &mjgame.ACKBC_Sddz_Card_Init{
		...
		LastOpt:         lastOpt,
		LastOptParam:    lastOptParam, //int32(r.IsHuang)
		IsHuang:         r.IsHuang,
		IsTouliao:       r.IsTouliao,
		IsMingW:         r.IsMingW,
		BaodaoState:     int32(r.BaodaoState),
		BaodaoNum:       int32(r.BaodaoNum),
		IsStrictWin:     r.IsStrictWin,
		IsSeleBaodao:    r.IsSeleBaodao,
		IsSeleStrictWin: r.IsSeleStrictWin,
		BaodaoCards:     r.BaodaoCards,
	}
...
//得到房间信息
func (r *RoomSrddz) GetRoomInfo() *mjgame.ACK_Room_Info {
...
func (r *RoomSrddz) GetRecordRoomInfo() interface{} {
	ack := &struct {
		RID        int32
		Type       int32
		Rule       []int32
...
//是否可以解散
func (r *RoomSrddz) GetDisApproveCount() int {
	var count int
	for _, v := range r.Votes {
		if v == DisApprove {
			count++
...
func (r *RoomSrddz) GetMultiWinInfo() ([]int, int) {
	//四人麻将最多三个人胡
	var huIndexes []int
	var huCardId = -1

	for i := 0; i < 3; i++ {
		lastTool := (r.StlCtrl).(*Srddz_Statement).Get(i)
...
func (r *RoomSrddz) GetMaxIndex(arr []int32) int {
	var maxIndex int
	var score int32
...
// 当前局可结束
func (r *RoomSrddz) RoundCanFinish() bool {
	var count int

	for _, v := range r.Seats {
		if v == nil || v.User == nil {
			fmt.Println("RoundCanFinish :: user nil")
			count++
			continue
...
// 以局数计算两局之间可否结束
func (r *RoomSrddz) ByRoundCanFinish() bool {
...
func (r *RoomSrddz) GetKickIndex() []int32 {
	var indexs []int32
...
// 杠牌操作
func (r *RoomSrddz) MoveKongList(uIndex int, tIndex int, cid, kongType int) {
	seat := r.Seats[uIndex]
...
func (r *RoomSrddz) MoveChowList(index int, cards []*rb.Card) {
	seat := r.Seats[index]
...
func (r *RoomSrddz) DealKongHu(cardID int) {
...
func (r *RoomSrddz) ClearRoomUserRoomId() {
	for _, v := range r.Seats {
		if v.User != nil {
			v.User.RoomId = 0
			v.User.GameType = nil
...
//设置玩家手中牌 用于测试环境
func (r *RoomSrddz) SetInitCards(uid string, cids []string) {
	for _, v := range r.Seats {
		if v.UID == uid { //&& r.CurMJIndex <= 61 && r.CurMJIndex > 0
			fmt.Printf("shezhiwanjiashoulidepai..")
			v.Cards.List = al.New()
...
//得到房间用户信息
func (r *RoomSrddz) GetRoomUser() *mjgame.ACK_Room_User {
...
//判断是否可以吃牌
func (r *RoomSrddz) CheckChow(pCard *rb.Card) {
...
//设置不能吃的牌
func (r *RoomSrddz) SetPassChowCards(pCard *rb.Card) {
