...
func GetSrddzStatement(mjType int32, bscore int, uid []string, room interface{}) *Srddz_Statement {
	smt := Srddz_Statement{}
...
type Srddz_Statement struct {
	BaseCtl st.StatementCtl
}

func (s *Srddz_Statement) Init() {
	s.BaseCtl.Init()
	s.AddType(T_Quan, "定圈")
}

func (s *Srddz_Statement) AddListCard(index int, listcard []int) {
	s.BaseCtl.AddListCard(index, listcard)
}

func (s *Srddz_Statement) AddRecord(record st.OnceRecord) {
	s.BaseCtl.AddRecord(record)
}

func (s *Srddz_Statement) AddTool(toolType int, index int, tindex int, val []int) {
	s.BaseCtl.AddTool(toolType, index, tindex, val)
}

func (s *Srddz_Statement) AddType(toolType int, msg string) {
	s.BaseCtl.AddType(toolType, msg)
}

func (s *Srddz_Statement) GetMsg(toolType int) string {
	return s.BaseCtl.GetMsg(toolType)
}

func (s *Srddz_Statement) Get(index int) *st.OnceRecord {
	return s.BaseCtl.GetPrvTool(index)
}

func (s *Srddz_Statement) GetForType(tooltype int) *st.OnceRecord {
	return s.BaseCtl.GetPrvToolForType(tooltype)
}

func (s *Srddz_Statement) GetForTypes(tooltypes []int) *st.OnceRecord {
	return nil
}

func (s *Srddz_Statement) GetTypeCount(uid int, tooltype int) int {
	return 0
}

func (s *Srddz_Statement) ToolCalc() *st.OnceRecord {
	return nil
}

func (s *Srddz_Statement) CalcTotal() {

}

// 判断是否胡牌
func (s *Srddz_Statement) CheckHu(pM []int) int {
...
func (s *Srddz_Statement) CheckChengBao(seatIndex int) ([]int, []int) {

	room := (*s.BaseCtl.RoomRef).(*RoomSrddz)
...
// 胡牌前的算番
// 判断是否一台起胡
func (s *Srddz_Statement) CheckCanWin(seatIndex int, args ...interface{}) bool {
	room := (*s.BaseCtl.RoomRef).(*RoomSrddz) //房间
...
// 算番 (自定义判断)
func (s *Srddz_Statement) FanCalc(seatIndex int, args ...interface{}) st.TotalResult {
...
//获取拉杠胡的杠牌以及杠牌座位
func (s *Srddz_Statement) GetLaGangHuInfo(seatIndex int) (int, int, int) {
	specialSid := -1
	gangIndex := -1
	gangCid := -1
	room := (*s.BaseCtl.RoomRef).(*RoomSrddz)
...
//获取送杠胡的杠牌以及杠牌座位
func (s *Srddz_Statement) GetSongGangHuInfo(seatIndex int) (int, int, int) {
	specialSid := -1
	gangIndex := -1
	gangCid := -1
	room := (*s.BaseCtl.RoomRef).(*RoomSrddz)
...
////获取还杠胡的杠牌以及杠牌座位
func (s *Srddz_Statement) GetHuanGangHuInfo(seatIndex int) (int, int, int) {
	specialSid := -1
	gangIndex := -1
	gangCid := -1
	room := (*s.BaseCtl.RoomRef).(*RoomSrddz)
