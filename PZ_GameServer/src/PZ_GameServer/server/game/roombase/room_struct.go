...
type SeatBase struct {
	...
	IsJiabei        bool          //是否加倍 三人斗地主
	IsMing          bool          //是否明牌 三人斗地主
	HaveJiao        bool          //是否叫过地主（或者是否抢过庄 拼十）
	JiaoFen         int           //玩家的叫分
	OutZha          int           //已经出的炸弹数量
	HaveJiabei      bool          //是否选择过加倍（或者是否下注 拼十）
	XiaZhu          int           //玩家的下注
	OutputNum       int           //玩家出牌的次数 结算春天和反春用
	LastOpt         int           //打牌阶段玩家最后的操作
	LastOptParam    []int32       //玩家最后操作的参数
	JoinPlay        bool          //玩家是否参与游戏
