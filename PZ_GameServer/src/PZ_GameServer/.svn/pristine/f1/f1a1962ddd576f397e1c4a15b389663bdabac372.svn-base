...
func GetXiangShanStatement(mjType int32, bscore int, uid []string, fanType int, room interface{}) *XiangShan_Statement {
	smt := XiangShan_Statement{}
	...
	smt.BaseCtl.FanType = fanType

	smt.Init()
	return &smt
}

type XiangShan_Statement struct {
	BaseCtl st.StatementCtl
}

func (s *XiangShan_Statement) Init() {
	s.BaseCtl.Init()
	s.AddType(T_Quan, "定圈")
}

func (s *XiangShan_Statement) AddListCard(index int, listcard []int) {
	s.BaseCtl.AddListCard(index, listcard)
}

func (s *XiangShan_Statement) AddRecord(record st.OnceRecord) {
	s.BaseCtl.AddRecord(record)
}

func (s *XiangShan_Statement) AddTool(toolType int, index int, tindex int, val []int) {
	s.BaseCtl.AddTool(toolType, index, tindex, val)
}

func (s *XiangShan_Statement) AddType(toolType int, msg string) {
	s.BaseCtl.AddType(toolType, msg)
}

func (s *XiangShan_Statement) GetMsg(toolType int) string {
	return s.BaseCtl.GetMsg(toolType)
}

func (s *XiangShan_Statement) Get(index int) *st.OnceRecord {
	return s.BaseCtl.GetPrvTool(index)
}

func (s *XiangShan_Statement) GetForType(tooltype int) *st.OnceRecord {
	return s.BaseCtl.GetPrvToolForType(tooltype)
}

func (s *XiangShan_Statement) GetForTypes(tooltypes []int) *st.OnceRecord {
	return nil
}

func (s *XiangShan_Statement) GetTypeCount(uid int, tooltype int) int {
	return 0
}

func (s *XiangShan_Statement) ToolCalc() *st.OnceRecord {
	return nil
}

func (s *XiangShan_Statement) CalcTotal() {

}

// 判断是否胡牌
func (s *XiangShan_Statement) CheckHu(pM []int) int {
...
func (s *XiangShan_Statement) CheckChengBao(seatIndex int) ([]int, []int) {

	room := (*s.BaseCtl.RoomRef).(*RoomXiangshan)
...
// 胡牌前的算番
// 判断是否一台起胡
func (s *XiangShan_Statement) CheckCanWin(seatIndex int, args ...interface{}) bool {
	room := (*s.BaseCtl.RoomRef).(*RoomXiangshan) //房间
...
// 算番 (自定义判断)
func (s *XiangShan_Statement) FanCalc(seatIndex int, args ...interface{}) st.TotalResult {
	...
	fanList := al.New()
	cb, cbfeng := s.CheckChengBao(seatIndex) //承包关系 cd是承包几倍 cbfeng是承包乘以的翻数
	...
	if !ischengbao { // 没有承包关系
		if zm.Tai > 0 { // 自摸
			for i := 0; i < room.Rules.SeatLen; i++ {
				if i != seatIndex {
					if haveDi == true { //有地胡的情况下
						if room.Seats[i].IsPutCard { //如果出了牌
							if s.BaseCtl.FanType == Qin10Hun6 {
								chengbaofeng[i] = int32((tai - 12) * -room.Rules.BaseScore)
							} else {
								chengbaofeng[i] = int32((tai - 16) * -room.Rules.BaseScore)
							}
...
//获取拉杠胡的杠牌以及杠牌座位
func (s *XiangShan_Statement) GetLaGangHuInfo(seatIndex int) (int, int, int) {
	specialSid := -1
	gangIndex := -1
	gangCid := -1
	room := (*s.BaseCtl.RoomRef).(*RoomXiangshan)
...
//获取送杠胡的杠牌以及杠牌座位
func (s *XiangShan_Statement) GetSongGangHuInfo(seatIndex int) (int, int, int) {
	specialSid := -1
	gangIndex := -1
	gangCid := -1
	room := (*s.BaseCtl.RoomRef).(*RoomXiangshan)
	if room.CurIndex == seatIndex {
		...
		for i := 1; i < 9; i++ {
			or1 := s.Get(i)
			if or1 != nil && or1.Tool.Index == room.CurIndex { //最后一个操作的人是自己
...
////获取还杠胡的杠牌以及杠牌座位
func (s *XiangShan_Statement) GetHuanGangHuInfo(seatIndex int) (int, int, int) {
	specialSid := -1
	gangIndex := -1
	gangCid := -1
	room := (*s.BaseCtl.RoomRef).(*RoomXiangshan)
