...
func GetXiZhouStatement(mjType int32, bscore int, uid []string, room interface{}) *XiZhou_Statement {
	smt := XiZhou_Statement{}
...
type XiZhou_Statement struct {
	BaseCtl st.StatementCtl
}

func (s *XiZhou_Statement) Init() {
	s.BaseCtl.Init()
	s.AddType(T_Quan, "定圈")
}

func (s *XiZhou_Statement) AddListCard(index int, listcard []int) {
	s.BaseCtl.AddListCard(index, listcard)
}

func (s *XiZhou_Statement) AddRecord(record st.OnceRecord) {
	s.BaseCtl.AddRecord(record)
}

func (s *XiZhou_Statement) AddTool(toolType int, index int, tindex int, val []int) {
	s.BaseCtl.AddTool(toolType, index, tindex, val)
}

func (s *XiZhou_Statement) AddType(toolType int, msg string) {
	s.BaseCtl.AddType(toolType, msg)
}

func (s *XiZhou_Statement) GetMsg(toolType int) string {
	return s.BaseCtl.GetMsg(toolType)
}

func (s *XiZhou_Statement) Get(index int) *st.OnceRecord {
	return s.BaseCtl.GetPrvTool(index)
}

func (s *XiZhou_Statement) GetForType(tooltype int) *st.OnceRecord {
	return s.BaseCtl.GetPrvToolForType(tooltype)
}

func (s *XiZhou_Statement) GetForTypes(tooltypes []int) *st.OnceRecord {
	return nil
}

func (s *XiZhou_Statement) GetTypeCount(uid int, tooltype int) int {
	return 0
}

func (s *XiZhou_Statement) ToolCalc() *st.OnceRecord {
	return nil
}

func (s *XiZhou_Statement) CalcTotal() {

}

// 判断是否胡牌
func (s *XiZhou_Statement) CheckHu(pM []int) int {
...
func (s *XiZhou_Statement) CheckChengBao(seatIndex int) ([]int, []int) {

	room := (*s.BaseCtl.RoomRef).(*RoomXiZhou)
...
// 胡牌前的算番
// 判断是否一台起胡
func (s *XiZhou_Statement) CheckCanWin(seatIndex int, args ...interface{}) bool {
	room := (*s.BaseCtl.RoomRef).(*RoomXiZhou) //房间
...
// 算番 (自定义判断)
func (s *XiZhou_Statement) FanCalc(seatIndex int, args ...interface{}) st.TotalResult {
...
//获取拉杠胡的杠牌以及杠牌座位
func (s *XiZhou_Statement) GetLaGangHuInfo(seatIndex int) (int, int, int) {
	specialSid := -1
	gangIndex := -1
	gangCid := -1
	room := (*s.BaseCtl.RoomRef).(*RoomXiZhou)
...
//获取送杠胡的杠牌以及杠牌座位
func (s *XiZhou_Statement) GetSongGangHuInfo(seatIndex int) (int, int, int) {
	specialSid := -1
	gangIndex := -1
	gangCid := -1
	room := (*s.BaseCtl.RoomRef).(*RoomXiZhou)
...
////获取还杠胡的杠牌以及杠牌座位
func (s *XiZhou_Statement) GetHuanGangHuInfo(seatIndex int) (int, int, int) {
	specialSid := -1
	gangIndex := -1
	gangCid := -1
	room := (*s.BaseCtl.RoomRef).(*RoomXiZhou)
