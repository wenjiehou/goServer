...
func (r *RoomModel) Query(ids []int, roomType int32) ([]Room, error) {
	...
	if err := r.db.Model(&Room{}).Preload("User").
		Where("id in (?) and type = ? and created_at > date_sub(now(),interval 3 day)", ids, roomType).
...
func (r *RoomModel) QueryAll(ids []int) ([]Room, error) {
	var rooms []Room
	if err := r.db.Model(&Room{}).Preload("User").
		Where("id in (?) and created_at > date_sub(now(),interval 3 day)", ids).
		Order("id desc").Find(&rooms).Error; err != nil {
		return nil, err
	}
