...
func GetNingBoStatement(mjType int32, bscore int, uid []string, room interface{}) *NingBo_Statement {
	smt := NingBo_Statement{}
...
type NingBo_Statement struct {
	BaseCtl st.StatementCtl
}

func (s *NingBo_Statement) Init() {
	s.BaseCtl.Init()
	s.AddType(T_Quan, "定圈")
}

func (s *NingBo_Statement) AddListCard(index int, listcard []int) {
	s.BaseCtl.AddListCard(index, listcard)
}

func (s *NingBo_Statement) AddRecord(record st.OnceRecord) {
	s.BaseCtl.AddRecord(record)
}

func (s *NingBo_Statement) AddTool(toolType int, index int, tindex int, val []int) {
	s.BaseCtl.AddTool(toolType, index, tindex, val)
}

func (s *NingBo_Statement) AddType(toolType int, msg string) {
	s.BaseCtl.AddType(toolType, msg)
}

func (s *NingBo_Statement) GetMsg(toolType int) string {
	return s.BaseCtl.GetMsg(toolType)
}

func (s *NingBo_Statement) Get(index int) *st.OnceRecord {
	return s.BaseCtl.GetPrvTool(index)
}

func (s *NingBo_Statement) GetForType(tooltype int) *st.OnceRecord {
	return s.BaseCtl.GetPrvToolForType(tooltype)
}

func (s *NingBo_Statement) GetForTypes(tooltypes []int) *st.OnceRecord {
	return nil
}

func (s *NingBo_Statement) GetTypeCount(uid int, tooltype int) int {
	return 0
}

func (s *NingBo_Statement) ToolCalc() *st.OnceRecord {
	return nil
}

func (s *NingBo_Statement) CalcTotal() {

}

// 判断是否胡牌
func (s *NingBo_Statement) CheckHu(pM []int) int {
...
func (s *NingBo_Statement) CheckChengBao(seatIndex int) ([]int, []int) {

	room := (*s.BaseCtl.RoomRef).(*RoomNingBo)
...
// 胡牌前的算番
// 判断是否一台起胡
func (s *NingBo_Statement) CheckCanWin(seatIndex int, args ...interface{}) bool {
	room := (*s.BaseCtl.RoomRef).(*RoomNingBo) //房间
...
// 算番 (自定义判断)
func (s *NingBo_Statement) FanCalc(seatIndex int, args ...interface{}) st.TotalResult {
...
//获取拉杠胡的杠牌以及杠牌座位
func (s *NingBo_Statement) GetLaGangHuInfo(seatIndex int) (int, int, int) {
	specialSid := -1
	gangIndex := -1
	gangCid := -1
	room := (*s.BaseCtl.RoomRef).(*RoomNingBo)
...
//获取送杠胡的杠牌以及杠牌座位
func (s *NingBo_Statement) GetSongGangHuInfo(seatIndex int) (int, int, int) {
	specialSid := -1
	gangIndex := -1
	gangCid := -1
	room := (*s.BaseCtl.RoomRef).(*RoomNingBo)
...
////获取还杠胡的杠牌以及杠牌座位
func (s *NingBo_Statement) GetHuanGangHuInfo(seatIndex int) (int, int, int) {
	specialSid := -1
	gangIndex := -1
	gangCid := -1
	room := (*s.BaseCtl.RoomRef).(*RoomNingBo)
